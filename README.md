...
- **[03/16/2026]** **v0.3.2** — Multi-claw support: IronClaw, PicoClaw, ZeroClaw, CoPaw, NanoClaw, NemoClaw, and opencode now supported alongside OpenClaw. NanoClaw connected via new `/v1/messages` Anthropic-compatible endpoint; NemoClaw via OpenShell inference routing. Added OpenRouter as a supported LLM platform.
...
1. **Choose your personal agent** — `openclaw`, `copaw`, `ironclaw`, `picoclaw`, `zeroclaw`, `nanoclaw`, `nemoclaw`, `opencode`, or `none` (MetaClaw will auto-configure it on start)
...
| `opencode` | [OpenCode](https://github.com/opencode/opencode) | Provides instructions on environment variables or patches `~/.opencode/.env` with `OPENAI_BASE_URL` and `OPENAI_API_KEY`. |
